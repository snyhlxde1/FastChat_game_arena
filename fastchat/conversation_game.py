...
class SeparatorStyle(IntEnum):
    ...
    ADD_COLON_TWO = auto()
    ADD_COLON_TWO_GAME_AKINATOR = auto()
...
class Conversation:
    ...
    game_secret = None
    end_prefix = None
    turn_limit_akinator = 20
    turn_limit_taboo = 5
    turn_limit_bluffing = 10
    turns = 0
    status = 'initialized' # for akinator game, should be one of: 'initialized', 'in-game', 'terminated'
    # --------------- gameplay related fields --------------- #
        ...
        if self.sep_style == SeparatorStyle.ADD_COLON_SINGLE:
            ...
            for i, (role, message) in enumerate(self.messages):
                ...
                if message:
                    ...
                    ret += role + ":"
            return ret
        elif self.sep_style == SeparatorStyle.ADD_COLON_TWO_GAME_AKINATOR:
            seps = [self.sep, self.sep2]
            #sep3 = "Above is the game history about the specific object.\n"
    ...
    def to_vertex_api_messages(self):
        ...
        for role, msg in self.messages[self.offset :]:
            if msg is not None:
                # game-related
                # disalbling vision understanding for now
    ...
    def to_replicate_api_messages(self):
        import replicate
        ...
        conv_template = "<|start_header_id|>{role}<|end_header_id|>\n\n"
        ...
        for role, msg in self.messages[self.offset :]:
            ...
            if msg is not None:
                if type(msg) is tuple:
                    # doesn't support vision for now
                    text, images = msg[0]
                    prompt_i = conv_template.format(role=role)
                    prompt_i = f"{prompt_i.strip() + msg}<|eot_id|>"
                else:
                    if role == 'user':
                        prompt_i = conv_template.format(role=role)
                        prompt_i = f"{prompt_i.strip() + msg}<|eot_id|>"
                        ...
                        prompt_i = conv_template.format(role=role)
                        prompt_i = f"{prompt_i.strip() + msg}"
            else:
                prompt_i = conv_template.format(role=role)
            all_conv.append(prompt_i)
        prompt_last = conv_template.format(role='assistant')
        all_conv.append(prompt_last)
                
        all_conv_seq = "".join(all_conv)
        system_template = "".join(system_template)
        ret = system_template + all_conv_seq
        return ret
...
# Vicuna v1.5 akinator game template
register_conv_template(
    Conversation(
        name="vicuna_v1.5_game_akinator",
        system_message="""You are an intelligent ASSISTANT tasked with playing a object guessing game. 
        Your goal is to to guess a very specific object that the user is thinking of. The rules are:
        (1) Each turn ask only one YES or NO question. Once the USER says START, you should start asking the first question. 
        (2) Guess the object if you are confident. You should use the following format: 'Are you thinking of $object?'
        (3) Stops the game if you think you found out the object.
        """,
        roles=("USER", "ASSISTANT"),
        sep_style=SeparatorStyle.ADD_COLON_TWO_GAME_AKINATOR,
        sep="\n",
        sep2=" ",
    )
)
